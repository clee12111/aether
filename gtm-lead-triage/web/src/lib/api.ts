...
if (typeof window !== "undefined") {
  console.log(`[GTM] API → ${API}${API_KEY ? " (key set)" : " (no key)"}`);
}
...
export async function apiGet<T>(path: string): Promise<T> {
  const res = await fetchWithTimeout(`${API}${path}`, { headers: authHeaders() }, 15_000);
  if (!res.ok) throw new Error(friendlyError(res.status, `${res.status} ${res.statusText}`));
  return res.json();
}
...
export function warmup(): void {
  fetch(`${API}/ready`, { method: "GET", headers: authHeaders() }).catch(() => {});
}
