...
export default function OpsDashboard() {
  ...
  return (
    <div className="flex flex-col min-h-[100dvh] h-[100dvh]">
      {/* Header */}
      <header className="border-b border-stone-200 bg-[var(--surface)] px-6 py-3 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-semibold text-sm text-stone-900">Ops</span>
        ...
        <Link href="/" className="text-xs font-medium text-stone-500 hover:text-stone-900 transition-colors">
          Lead Form
        </Link>
      </header>

      {/* Filter bar */}
      <div className="border-b border-stone-200 bg-[var(--surface)] px-6 py-2 flex items-center gap-2 text-xs shrink-0 overflow-x-auto">
        <button
          onClick={() => setTierFilter(null)}
          className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md transition-colors ${!tierFilter ? "bg-stone-900 text-white" : "text-stone-500 hover:bg-stone-100"}`}
        >
        ...
        {(["hot", "warm", "cold", "disqualified"] as const).map((t) => (
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md transition-colors ${tierFilter === t ? "bg-stone-900 text-white" : "text-stone-500 hover:bg-stone-100"}`}
          >
            <span className={`w-1.5 h-1.5 rounded-full ${tierFilter === t ? "bg-[var(--surface)]" : TIER_DOT[t]}`} />
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <select
            ...
            className="px-2 py-1 text-xs bg-stone-50 border border-stone-200 rounded-md focus:outline-none focus:ring-1 focus:ring-indigo-600/20 text-stone-600"
          >
          ...
          <input
            ...
            placeholder="Search..."
            className="w-32 px-2.5 py-1 text-xs bg-stone-50 border border-stone-200 rounded-md focus:outline-none focus:ring-1 focus:ring-indigo-600/20 placeholder:text-stone-400"
          />
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-80 border-r border-stone-200 bg-[var(--surface)] overflow-y-auto shrink-0">
          {loading && <><LeadSkeleton /><LeadSkeleton /><LeadSkeleton /></>}
          {!loading && filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center px-6">
              <p className="text-sm text-stone-500 mb-1">{leads.length === 0 ? "No leads yet" : "No matches"}</p>
          ...
          {filtered.map((lead) => (
            <button
              ...
              className={`w-full text-left px-4 py-3 border-b border-stone-100 transition-colors ${selectedEmail === lead.email ? "bg-indigo-50/50 border-l-2 border-l-indigo-500" : "hover:bg-stone-50"}`}
            >
              <div className="flex items-center justify-between mb-0.5">
                <span className="text-sm font-medium text-stone-900 truncate pr-2">{lead.name || lead.email}</span>
                <div className="flex items-center gap-2 shrink-0">
                  {lead.score !== undefined && (
                    <span className="text-[10px] font-mono text-stone-400">{lead.score}</span>
                  )}
                  {lead.tier && (
              ...
              <div className="text-xs text-stone-400 truncate">{lead.email}</div>
              {lead.company && <div className="text-[10px] text-stone-400 mt-0.5 truncate">{lead.company}</div>}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto bg-stone-50/50">
          {!selectedEmail && !detailLoading && (
            <div className="flex flex-col items-center justify-center h-full text-center px-6">
              <p className="text-sm text-stone-500">Select a lead to view its triage trace</p>
            </div>
          )}
          ...
          {selectedEmail && !detailLoading && !selectedRun && selectedLead && (
            <div className="flex flex-col items-center justify-center h-full text-center px-6">
              <p className="text-sm font-medium text-stone-700 mb-1">{selectedLead.name || selectedEmail}</p>
              <p className="text-xs text-stone-400 mb-3">{selectedLead.tier && `${selectedLead.tier} / ${selectedLead.route || ""}`}</p>
              <p className="text-xs text-stone-400">No triage trace found for this contact.</p>
              <p className="text-[10px] text-stone-300 mt-1">The contact exists in CRM but has no linked run in the trace store.</p>
            </div>
          )}
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                <div className="flex items-start justify-between">
                  <div>
                    <h2 className="text-lg font-semibold tracking-tight text-stone-900">{selectedLead.name || selectedEmail}</h2>
                    <p className="text-xs text-stone-400 mt-0.5">{selectedEmail}{selectedLead.company ? ` · ${selectedLead.company}` : ""}</p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    {selectedLead.tier && (
                    ...
                    <button
                      ...
                      disabled={deleting}
                      className="text-[10px] font-medium text-stone-400 hover:text-red-600 transition-colors disabled:opacity-50"
                ...
                {/* Stats bar */}
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2 text-[10px] text-stone-400 font-mono">
                  ...
                  {tr?.provider_used && (
                    <span className={tr.provider_used === "openai" ? "text-emerald-500" : "text-stone-400"}>
                      {tr.provider_used}
                    </span>
                  )}
                  ...
                  {config?.langfuse_enabled && config.langfuse_host && (
                    <span className="text-stone-400 font-sans cursor-default" title="Langfuse traces are linked to the operator's account — in production, this opens your team's Langfuse dashboard">Langfuse</span>
                  )}
                  {config?.crm_backend === "hubspot" ? (
                    <span className="text-stone-400 font-sans cursor-default" title="HubSpot contacts are linked to the operator's account — in production, this opens your team's HubSpot dashboard">HubSpot</span>
                  ) : (
              ...
              {tr?.outreach?.subject && (
                <div className="bg-[var(--surface)] border border-stone-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-xs font-semibold text-stone-500 uppercase tracking-wide">Draft Outreach</h3>
                  ...
                  <p className="text-sm font-medium text-stone-800 mb-1">{tr.outreach.subject}</p>
                  <p className="text-xs text-stone-500 leading-relaxed whitespace-pre-wrap">{tr.outreach.body}</p>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {tr?.score && (
                  <div className="bg-[var(--surface)] border border-stone-200 rounded-lg p-4">
                    <h3 className="text-xs font-semibold text-stone-500 uppercase tracking-wide mb-3">Score</h3>
                    <div className="flex items-baseline gap-2 mb-3">
                      <span className="text-3xl font-bold text-stone-900 font-mono">{tr.score.points ?? tr.score.rule_points}</span>
                    ...
                    <div className="space-y-1.5 text-[11px] text-stone-500">
                      ...
                      {tr.score.reason && (
                        <p className="text-[10px] text-stone-400 mt-2 pt-2 border-t border-stone-100 leading-relaxed font-mono">{tr.score.reason}</p>
                      )}
                    </div>
                    {tr.score.llm_reason && (
                      <p className="text-[10px] text-stone-400 mt-2 leading-relaxed italic">{tr.score.llm_reason}</p>
                    )}
                  </div>
                )}
                ...
                {tr?.enrichment && (
                  <div className="bg-[var(--surface)] border border-stone-200 rounded-lg p-4">
                    <h3 className="text-xs font-semibold text-stone-500 uppercase tracking-wide mb-3">Enrichment</h3>
                    <div className="space-y-2 text-[11px]">
                      {([
                        ...
                        <div key={k as string} className="flex justify-between">
                          <span className="text-stone-500">{k as string}</span>
              ...
              <div>
                <h3 className="text-xs font-semibold text-stone-500 uppercase tracking-wide mb-2">Triage trace</h3>
                <div className="space-y-1">
                  {phases.map((phase, pi) => {
                    ...
                    return (
                      <div key={pi} className="bg-[var(--surface)] border border-stone-200 rounded-lg overflow-hidden">
                        <button
                          onClick={() => togglePhase(pi)}
                          className="w-full flex items-center gap-2.5 px-3.5 py-2 text-left hover:bg-stone-50 transition-colors"
                        >
                          <div className={`w-2 h-2 rounded-full shrink-0 ${isEnd ? "bg-emerald-500" : isStart ? "bg-stone-300" : "bg-indigo-500"}`} />
                          <span className="text-sm font-medium text-stone-800 flex-1">{phase.name}</span>
                          {phaseDuration > 0 && (
                            <span className="text-[10px] text-stone-400 font-mono">{phaseDuration}ms</span>
                          )}
                          <span className="text-[10px] text-stone-400">{phase.events.length} step{phase.events.length !== 1 ? "s" : ""}</span>
                          <svg className={`w-3 h-3 text-stone-400 transition-transform ${isExpanded ? "rotate-90" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
                          </svg>
                        </button>
                        {isExpanded && (
                          <div className="border-t border-stone-100 px-3.5 py-2 space-y-1.5">
                            {phase.events.map((e, ei) => {
                              const typeColors: Record<string, string> = {
                                llm_call: "bg-indigo-50 text-indigo-700",
                                tool_call: "bg-emerald-50 text-emerald-700",
                                tool_response: "bg-stone-100 text-stone-600",
                                run_start: "bg-stone-100 text-stone-600",
                                run_end: "bg-emerald-50 text-emerald-700",
                              };
                              return (
                                <div key={ei} className="text-[11px]">
                                  <div className="flex items-center gap-2">
                                    <span className={`px-1.5 py-0.5 rounded font-mono font-medium ${typeColors[e.event_type] || "bg-stone-100 text-stone-600"}`}>
                                      {e.event_type.replace("_", " ")}
                                    </span>
                                    <span className="text-stone-500">{e.agent}</span>
                                    {e.duration_ms != null && e.duration_ms > 0 && (
                                      <span className="text-stone-400 font-mono ml-auto">{e.duration_ms}ms</span>
                                    )}
                                    {e.input_tokens != null && e.input_tokens > 0 && (
                                      <span className="text-stone-400 font-mono">{e.input_tokens}+{e.output_tokens}tok</span>
                                    )}
                                  </div>
                                  <pre className="mt-1 text-[10px] text-stone-400 font-mono whitespace-pre-wrap break-words max-h-32 overflow-y-auto leading-relaxed">
